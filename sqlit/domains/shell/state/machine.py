...
class UIStateMachine:
    ...
    def generate_help_text(self) -> str:
        ...
        lines.append(binding(":help, :h", "Show keyboard shortcuts"))
        lines.append(binding(":q, :quit, :exit", "Quit sqlit"))
        lines.append(binding(":connect, :c", "Open connection picker"))
        lines.append(binding(":disconnect, :dc", "Disconnect from current server"))
        lines.append(binding(":theme", "Open theme selection"))
        lines.append(binding(":run, :r", "Execute query"))
        lines.append(binding(":run!, :r!", "Execute query (stay in INSERT)"))
        lines.append(binding(":process-worker, :worker", "Toggle process worker"))
        lines.append(binding(":worker info", "Show process worker status"))
        lines.append(binding(":wd <ms|off>", "Set UI stall watchdog threshold"))
        lines.append(binding(":wd list", "Show UI stall warnings"))
        lines.append(binding(":wd clear", "Clear UI stall log"))
        lines.append(binding(":debug on|off", "Toggle debug event logging"))
        lines.append(binding(":debug", "Show debug status"))
        lines.append(binding(":debug list", "Show debug events"))
        lines.append(binding(":debug clear", "Clear debug event log"))
